...
```bash
curl -X POST "https://api.exemple.com/predict/2d" \
  -F "file=@image.png"
```
...
```bash
curl -X POST "https://api.exemple.com/predict/3d" \
  -F "file=@image.png"
```
...
```json
{
  ...
  "data": {
    "task_id": 127,
    "pipeline": "2d",
    "image_url": "http://images.exemple.com/2d/uploads/20260316/abc123.png",
...
```bash
curl "https://api.exemple.com/tasks/127?pipeline=2d"
```

## Exemple 3D

```bash
curl "https://api.exemple.com/tasks/127?pipeline=3d"
```
...
```bash
curl -X POST "https://api.exemple.com/admin/reload-model/2d"
```

ou

```bash
curl -X POST "https://api.exemple.com/admin/reload-model/3d"
```
...
```bash
curl https://api.exemple.com//health/2d
curl https://api.exemple.com//health/3d
```
...
| `DB_PASS` | `CHANGE_ME` | Mot de passe DB |
...
| `IMAGES_PUBLIC_BASE` | `http://images.exemple.com` | URL publique images |
