...
def evaluate_by_category(
    ...
    for cat in sorted(by_cat):
        ...
        mean_normal = metrics.get("score_stats_normal", {}).get("mean")
        std_normal = metrics.get("score_stats_normal", {}).get("std", 0)
        mean_anomaly = metrics.get("score_stats_anomaly", {}).get("mean")
        if mean_normal is not None:
            cat_threshold = mean_normal + 2 * std_normal
        else:
        ...
        summary[cat] = {
            ...
            "threshold": round(float(cat_threshold), 6),
        }
        print(
            f"  [{cat}] n={summary[cat]['n']} "
            f"F1={summary[cat]['f1']} "
            f"AUROC={summary[cat]['auroc']} "
            f"threshold={cat_threshold:.4f}"
        )
