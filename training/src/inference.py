...
_PREPROCESS = transforms.Compose([
    transforms.Resize((224, 224)),  # même taille fit/predict
    transforms.ToTensor(),
...
def _build_backbone(model_dir: Optional[str] = None) -> nn.Module:
    """
    Build ResNet18 backbone with fc=Identity.
    If <model_dir>/backbone_finetuned.pt exists, load it.
...
# Cache to avoid reloading per prediction
...
def _pil_to_tensor(pil_img) -> torch.Tensor:
    return _PREPROCESS(pil_img).unsqueeze(0)  # [1,3,224,224]
...
def image_to_embedding(pil_img, model_dir: Optional[str] = None) -> np.ndarray:
    """
    Convert PIL -> embedding (512,) L2-normalized.
    Uses finetuned backbone if present in model_dir.
    """
    x = _pil_to_tensor(pil_img).to(_DEVICE)
    embedder = get_embedder(model_dir)

    with torch.no_grad():
        z = embedder(x).squeeze(0)  # [512]
        z = z / (torch.norm(z) + 1e-12)  # L2 normalize
...
def _forward_to_layer(
    ...
    """
    Forward ResNet up to an intermediate layer.
    Returns feature map [B, C, H, W].

    Supported: layer1, layer2, layer3, layer4, layer23
    """
...
def image_to_patch_embeddings(
    ...
    """
    Convert PIL image -> patch embeddings [P, C] L2-normalized.
    """
    x = _pil_to_tensor(pil_img).to(_DEVICE)
...
def load_reference_bank(model_dir: str) -> Tuple[np.ndarray, str]:
    ...
    Returns:
      (bank, mode)  where mode is "patch" or "global"

    Priority: patch_bank.npy (-> mode=patch) > embeddings.npy (-> mode=global)
    """
    candidates = [
...
def load_bank_meta(model_dir: str) -> Dict:
    """Load patch_bank_meta.json if present (contains feature_layer, backbone, etc.)."""
...
def fit_reference_bank(
    ...
    """
    Fit from DB using PFEDataManager + MVTecDataset(split=train, only_good=True).
    Saves embeddings.npy in model_dir.

    IMPORTANT:
      - If <model_dir>/backbone_finetuned.pt exists, it will be used automatically.
      - Dataset uses the same preprocess as predict (ImageNet normalize + 224).
    """
    settings = Settings.from_yaml(config_path)
    dm = PFEDataManager(settings=settings)

    dataset = MVTecDataset(dm, table_name=table_name, split="train", only_good=True)
    dataset.transform = _PREPROCESS  # same as predict
    ...
    embedder = get_embedder(model_dir)  # will load finetuned backbone if present

    embs = []
    with torch.no_grad():
        for imgs, _ in loader:
            imgs = imgs.to(_DEVICE, non_blocking=True)
            feats = embedder(imgs)  # [B,512]
            feats = feats / (torch.norm(feats, dim=1, keepdim=True) + 1e-12)  # safety normalize
...
def predict_anomaly(
    ...
    """
    Score = mean cosine distance to k nearest neighbors in reference bank.

    If model_dir is provided and contains backbone_finetuned.pt, it will be used.
    """
    if bank is None or len(bank) == 0:
        raise ValueError("Reference bank is empty. Run fit_reference_bank() first.")

    emb = image_to_embedding(pil_img, model_dir=model_dir)[None, :]  # [1,512]
...
def aggregate_patch_scores(
    ...
    Supported: max, mean, topk_mean, median
    """
...
def predict_patch_anomaly(
    ...
    """
    Patch-level anomaly prediction (compatible with training pipeline).
    ...
    """
    # Build NN index on the reference patch bank
    ...
    # Extract query patches from the input image
...
def upsample_patch_map(
    ...
    """Upsample [H, W] patch map to [out_h, out_w]."""
...
def heatmap_to_rgb(heatmap: np.ndarray) -> np.ndarray:
    """Convert [H, W] normalized heatmap to RGB uint8 using jet colormap."""
    import matplotlib
    matplotlib.use("Agg")
